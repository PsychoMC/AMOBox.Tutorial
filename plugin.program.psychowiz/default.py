...
addon_id = 'plugin.program.psychowiz'
ADDON = xbmcaddon.Addon(id=addon_id)
AddonID='plugin.program.psychowiz'
AddonTitle="[COLOR aqua]P[/COLOR][COLOR white]sycho Wizard[/COLOR]"
...
EXCLUDES2     = ['plugin.program.psychowiz','script.module.addon.common','packages']
EXCLUDES     = ['plugin.program.psychowiz','script.module.addon.common']
...
def INDEX():
	...
	addDir('[COLOR aqua]P[/COLOR][COLOR white]sycho Builds[/COLOR]',BASEURL,20,ART+'amoboxwiz.png',FANART,'')
	addDir('[COLOR aqua]C[/COLOR][COLOR white]omunidade Builds[/COLOR]',BASEURL,100,ART+'amoboxwizcomm.png',FANART,'')
	addDir('[COLOR aqua]A[/COLOR][COLOR white]PK Store[/COLOR]',BASEURL,40,ART+'apkstore.png',FANART,'')
	addDir('[COLOR aqua]M[/COLOR][COLOR white]anutencao[/COLOR]',BASEURL,30,ART+'maintenance.png',FANART,'')
	addDir('[COLOR aqua]C[/COLOR][COLOR white]B TOOLS[/COLOR]',BASEURL,50,ART+'Backup.png',FANART,'')
	addDir('[COLOR aqua]F[/COLOR][COLOR white]acebook[/COLOR]',BASEURL,8,ART+'facebook.png',FANART,'')
...
def facebook():
    TextBoxes('[COLOR aqua]P[/COLOR][COLOR white]sycho Wizard[/COLOR]', 'Descobre-nos em: [COLOR blue]http://bit.ly/PsychoFacebook[/COLOR]')
...
def COMMUNITY_BACKUP():
    ...
    exclude_dirs_full =  ['plugin.program.psychowiz']
    exclude_files_full = ["xbmc.log","xbmc.old.log","kodi.log","kodi.old.log",'.DS_Store','.setup_complete','XBMCHelper.conf']
    exclude_dirs =  ['plugin.program.psychowiz', 'cache', 'system', 'Thumbnails', "peripheral_data",'library','keymaps']
    ...
    message_header2 = "Creaando AMObox Custom Build"
    message1 = "Arquivando..."
    message2 = ""
...
def ARCHIVE_FILE(sourcefile, destfile):
    ...
    for base, dirs, files in os.walk(sourcefile):
        for file in files:
            ...
            if not 'temp' in dirs:
                if not 'plugin.program.psychowiz' in dirs:
                   import time
                   FORCE= '01/01/1980'
...
def RESTORE_LOCAL_COMMUNITY():
    ...
    if exitfunction == 1:
        ...
        dialog.ok('[COLOR=blue][B]AMO[/B][/COLOR][COLOR=green]box[/COLOR] Custom Builds Tool','Processo completo.',' Escolha Force Close agora','para reiniciar o Kodi e as alteracoes terem efeito.')        
        killxbmc()
#---------------------------------------------------------------------------------------------------
...
def WipeXBMC():
    if skin!= "skin.confluence":
        ...
        if choice == 1:
            ...
            title = urllib.quote_plus(vq)
            backup_zip = xbmc.translatePath(os.path.join(mybackuppath,title+'.zip'))
            exclude_dirs_full =  ["plugin.program.psychowiz", "plugin.video.adrxbmccustombuildswizard"]
            exclude_files_full = ["xbmc.log","xbmc.old.log","kodi.log","kodi.old.log",'.DS_Store','.setup_complete','XBMCHelper.conf']
...
def FRESHSTART(params):
    if skin!= "skin.confluence":
        dialog.ok('[COLOR=blue][B]AMO[/B][/COLOR][COLOR=green]box[/COLOR][COLOR=white] Wizard[/COLOR] ','Please switch to the default Confluence skin','before performing a wipe.','')
        xbmc.executebuiltin("ActivateWindow(appearancesettings)")
        return
    else:
    ...
    if choice2 == 0:
        ...
        dp.create("[COLOR=blue][B]AMO[/B][/COLOR][COLOR=green]box[/COLOR][COLOR=white] Wizard[/COLOR]","Wiping Install",'', 'Please Wait')
        try:
            for root, dirs, files in os.walk(HOME,topdown=True):
                dirs[:] = [d for d in dirs if d not in EXCLUDES]
                for name in files:
    ...
    REMOVE_EMPTY_FOLDERS()
    REMOVE_EMPTY_FOLDERS()
    dialog.ok('[COLOR=blue][B]AMO[/B][/COLOR][COLOR=green]box[/COLOR][COLOR=white] Wizard[/COLOR]','Wipe Successful, please restart XBMC/Kodi for changes to take effect.','','')
    killxbmc()
...
def FRESHSTART2(params):
    dp.create("[COLOR aqua]P[/COLOR][COLOR white]sycho Wizard[/COLOR]","Limpando o Kodi",'', 'Aguarde...')
    try:
        for root, dirs, files in os.walk(HOME,topdown=True):
            dirs[:] = [d for d in dirs if d not in EXCLUDES]
            for name in files:
