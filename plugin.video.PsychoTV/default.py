...
def makeRequest(url, headers=None):
        try:
            ...
            if hasattr(e, 'code'):
                ...
                xbmc.executebuiltin("XBMC.Notification([COLOR aqua]P[/COLOR][COLOR white]sychoTV[/COLOR],We failed with error code - "+str(e.code)+",10000,"+icon+")")
            elif hasattr(e, 'reason'):
                addon_log('We failed to reach a server.')
                addon_log('Reason: %s' %e.reason)
                xbmc.executebuiltin("XBMC.Notification([COLOR aqua]P[/COLOR][COLOR white]sychoTV[/COLOR],We failed to reach a server. - "+str(e.reason)+",10000,"+icon+")")
...
def GetSublinks(name,url,iconimage,fanart):
    ...
    if c==1:
        try:
         ...
         rNo=dialog.select('[COLOR aqua]P[/COLOR][COLOR white]sychoTV[/COLOR] Select A Source', List)   
         if rNo>=0:
             rName=name
             rURL=str(ListU[rNo])
...
def urlsolver(url):
    ...
    try:
        ...
        xbmc.executebuiltin("XBMC.Notification([COLOR aqua]P[/COLOR][COLOR white]sychoTV[/COLOR],Please enable Update Commonresolvers to Play in Settings. - ,10000)")  
    resolved=genesisresolvers.get(url).result
    if url == resolved or resolved is None:
        #import
        xbmc.executebuiltin("XBMC.Notification([COLOR aqua]P[/COLOR][COLOR white]sychoTV[/COLOR],Using Urlresolver module.. - ,5000)")  
        import urlresolver
...
def search(site_name,search_term=None):
    ...
    if 'youtube' in site_name:
        ...
        if res == 'True':
            ...
            xbmc.executebuiltin("XBMC.Notification([COLOR aqua]P[/COLOR][COLOR white]sychoTV[/COLOR],No IMDB match found ,7000,"+icon+")")
...
def addLink(url,name,iconimage,fanart,description,genre,date,showcontext,playlist,regexs,total,setCookie=""):
        ...
        if showcontext:
            ...
            if showcontext == 'fav':
                contextMenu.append(
                    ('Remove from [COLOR aqua]P[/COLOR][COLOR white]sychoTV[/COLOR] Favorites','XBMC.RunPlugin(%s?mode=6&name=%s)' 
                     %(sys.argv[0], urllib.quote_plus(name)))
                     )
                ...
                contextMenu.append(('Add to [COLOR aqua]P[/COLOR][COLOR white]sychoTV[/COLOR] Favorites','XBMC.RunPlugin(%s)' %fav_params)) 
            liz.addContextMenuItems(contextMenu)
