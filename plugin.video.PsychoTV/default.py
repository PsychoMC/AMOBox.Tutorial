...
addon_id = 'plugin.video.PsychoTV'
ADDON = xbmcaddon.Addon(id=addon_id)
AddonID='plugin.video.PsychoTV'
AddonTitle="[COLOR aqua]P[/COLOR][COLOR white]sychoTV[/COLOR]"
...
def addon_log(string):
    if debug == 'true':
        xbmc.log("[addon.all-%s]: %s" %(addon_version, string))
...
def makeRequest(url, headers=None):
        try:
            ...
            if hasattr(e, 'code'):
                ...
                xbmc.executebuiltin("XBMC.Notification(ALL,We failed with error code - "+str(e.code)+",10000,"+icon+")")
            elif hasattr(e, 'reason'):
                addon_log('We failed to reach a server.')
                addon_log('Reason: %s' %e.reason)
                xbmc.executebuiltin("XBMC.Notification(ALL,We failed to reach a server. - "+str(e.reason)+",10000,"+icon+")")
...
def getSources():
        try:
            if os.path.exists(favorites) == True:
                addDir('[COLOR white][COLOR aqua][B]• [/B] [/COLOR][B]Favoritos[/B][/COLOR]','url',4,os.path.join(home, 'resources', 'favorite.png'),FANART,'','','','')
            ...
            if addon.getSetting("searchotherplugins") == "true":
                addDir('Procurar outros Plugins','Procurar Plugins',25,icon,FANART,'','','','')
...
def playmediawithproxy(media_url, name, iconImage,proxyip,port):
    ...
    progress.create('Progresso', 'Abrindo com proxy personalizado')
    progress.update( 10, "", "configuração de proxy..", "" )
    proxyset=False
    existing_proxy=''
    try:
        ...
        print 'saindo do proxy',existing_proxy
        #read and set here
        setKodiProxy( proxyip + ':' + port+':0')

        print 'definição proxy completo', getConfiguredProxy()
        proxyset=True
...
def createM3uForDash(url,useragent=None):
    ...
    #return 'special://home/Downloads/test.m3u8'
    return source_file
...
def urlsolver(url):
    ...
    if host:
        ...
        xbmc.executebuiltin("XBMC.Notification([COLOR aqua]P[/COLOR][COLOR white]sychoTV[/COLOR],Urlresolver donot support this domain. - ,5000)")
    return resolver
...
def download_file(name, url):
        if addon.getSetting('save_location') == "":
            xbmc.executebuiltin("XBMC.Notification('[COLOR aqua]P[/COLOR][COLOR white]sychoTV[/COLOR]','Choose a location to save files.',15000,"+icon+")")
            addon.openSettings()
        params = {'url': url, 'download_path': addon.getSetting('save_location')}
        downloader.download(name, params)
        dialog = xbmcgui.Dialog()
        ret = dialog.yesno('[COLOR aqua]P[/COLOR][COLOR white]sychoTV[/COLOR]', 'Do you want to add this file as a source?')
        if ret:
...
#            print 'parentalblockedpin',parentalblockedpin
            ...
            if showcontext == 'source':
                ...
                contextMenu.append(('Remove from [COLOR aqua]P[/COLOR][COLOR white]sychoTV[/COLOR] Favorites','XBMC.RunPlugin(%s?mode=6&name=%s)'
            ...
            if not name in FAV:
                contextMenu.append(('Add to [COLOR aqua]P[/COLOR][COLOR white]sychoTV[/COLOR] Favorites','XBMC.RunPlugin(%s?mode=5&name=%s&url=%s&iconimage=%s&fanart=%s&fav_mode=%s)'
   ...
   xbmc.executebuiltin("Container.SetViewMode(%s)"%viewmode)
    ...
    if not (keyboard.isConfirmed() == False):
        ...
        if newStr==parentalblockedpin:
            ...
            xbmc.executebuiltin("XBMC.Notification([COLOR aqua]P[/COLOR][COLOR white]sychoTV[/COLOR],Parental Block Disabled,5000,"+icon+")")
        else:
            xbmc.executebuiltin("XBMC.Notification([COLOR aqua]P[/COLOR][COLOR white]sychoTV[/COLOR],Wrong Pin??,5000,"+icon+")")
    ...
    xbmc.executebuiltin("XBMC.Notification([COLOR aqua]P[/COLOR][COLOR white]sychoTV[/COLOR],Parental block enabled,5000,"+icon+")")
